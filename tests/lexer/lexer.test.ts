...
import { TokenType } from "../../src/lexer/consts/token-type";
...
describe("Lexer", () => {
  test("tokenizes basic expressions correctly", () => {
    ...
    expect(tokens[7].type).toBe(TokenType.DATATYPE);
    expect(tokens[8].type).toBe(TokenType.BOOLEAN_VALUE);
    expect(tokens[9].type).toBe(TokenType.LOGICAL_OPERATOR);
  });
